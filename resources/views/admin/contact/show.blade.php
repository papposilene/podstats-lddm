@extends('layouts.admin')
@section('title', $contact->uname)

@section('content')            
<div class="d-flex justify-content-between flex-wrap flex-md-nowrap align-items-center pt-4 pb-2 mb-3 border-bottom">
    <h1 class="h2 text-white">
        {{ $contact->uname }}
    </h1>
    <div class="btn-toolbar mb-2 mb-md-0">
        <div class="btn-group mr-2">
            @can('create')
            <a href="{{ route('admin.contact.edit', ['uuid' => $contact->uuid]) }}" class="btn btn-sm btn-primary" role="button">
                <i class="fas fa-edit" aria-hidden="true" title="@ucfirst(__('app.contactUpdate'))"></i>
                <span class="sr-only">@ucfirst(__('app.contactUpdate'))</span>
            </a>
            @endcan
            <a href="{{ route('public.contact.show', ['uuid' => $contact->uuid]) }}" role="button" class="btn btn-sm btn-info" target="_blank">
                <i class="fas fa-external-link-alt" aria-hidden="true"></i>
            </a>
        </div>
        <form action="{{ route('admin.contact.search') }}" method="POST">
            @csrf
            <div class="input-group input-group-sm">
                <input type="text" class="form-control bg-dark border border-secondary text-white" name="q" placeholder="@ucfirst(__('app.search'))" aria-label="@ucfirst(__('app.search'))">
                <div class="input-group-append">
                    <button type="submit" class="bg-dark border border-secondary btn-sm text-white">
                        <i class="fas fa-search" aria-hidden="true" aria-label="@ucfirst(__('app.search'))"></i>
                    </button>
                </div>
            </div>
        </form>
    </div>
</div>

<div class="row">
    <div class="col-8">
        @if (count($contact->hasGames) > 0)
        <h2 class="text-white">
            <i class="fas fa-gamepad" aria-hidden="true" title="@ucfirst(__('app.gamesList'))"></i>
            @ucfirst(__('app.gamesList'))
            @can('update')
            <button type="button" class="btn btn-sm btn-primary float-right" data-toggle="modal" data-target="#modalGameCreate">
                @ucfirst(__('app.tracksLink'))
            </button>
            @endcan
        </h2>

        <div class="table-responsive">
            <table class="table table-dark table-striped table-hover">
                <thead>
                    <tr>
                        <th class="text-center">#</th>
                        <th class="text-center">@ucfirst(__('app.studio'))</th>
                        <th class="text-center">@ucfirst(__('app.gameTitle'))</th>
                        <th class="text-center">@ucfirst(__('app.gameDate'))</th>
                        <th class="text-center">@ucfirst(__('app.profession'))</th>
                        <th class="text-center">@ucfirst(__('app.actions'))</th>
                    </tr>
                </thead>
                <tbody>
                    @foreach($contact->hasGames as $game)
                    <tr>
                        <td class="text-center">{{ $loop->iteration }}</td>
                        <td>
                            @if ($game->hasGame->studio_uuid)
                            <a href="{{ route('admin.studio.show', ['uuid' => $game->hasGame->studio_uuid]) }}" class="text-white">
                                {{ $game->hasGame->createdBy->studio }}
                            </a>
                            @else
                            @ucfirst(__('app.studioUnknown'))
                            @endif
                        </td>
                        <td>
                            <a href="{{ route('admin.game.show', ['uuid' => $game->hasGame->uuid]) }}" class="text-white">
                                {{ $game->hasGame->title }}
                            </a>
                        </td>
                        <td class="text-center">
                            @if ($game->hasGame->released_on)
                            @date($game->hasGame->released_on)
                            @endif
                        </td>
                        <td>
                            {{ $game->hasProfession->profession }}
                        </td>
                        <td class="text-center">
                            <a href="{{ route('admin.game.show', ['uuid' => $game->hasGame->uuid]) }}" class="btn btn-sm btn-primary">
                                <i class="fas fa-search" aria-hidden="true" title="@ucfirst(__('app.showData'))"></i>
                            </a>
                        </td>
                    </tr>
                    @endforeach
                </tbody>
            </table>
        </div>
        @endif

        @if (count($contact->hasTracks) > 0)
        <h2 class="text-white">
            <i class="fas fa-music" aria-hidden="true" title="@ucfirst(__('app.tracksList'))"></i>
            @ucfirst(__('app.tracksList'))
            @can('update')
            <button type="button" class="btn btn-sm btn-primary float-right" data-toggle="modal" data-target="#modalTrackLink">
                @ucfirst(__('app.tracksLink'))
            </button>
            @endcan
        </h2>

        <div class="table-responsive">
            <table class="table table-dark table-striped table-hover">
                <thead>
                    <tr>
                        <th class="text-center">#</th>
                        <th class="text-center">@ucfirst(__('app.gameTitle'))</th>
                        <th class="text-center">@ucfirst(__('app.trackTitle'))</th>
                        <th class="text-center">@ucfirst(__('app.trackDuration'))</th>
                        <th class="text-center">@ucfirst(__('app.actions'))</th>
                    </tr>
                </thead>
                <tbody>
                    @foreach($contact->hasTracks as $track)
                    <tr>
                        <td class="text-center">{{ $loop->iteration }}</td>
                        <td>
                            <a href="{{ route('admin.game.show', ['uuid' => $track->composedFor->uuid]) }}" class="text-white">
                                {{ $track->composedFor->title }}
                            </a>
                        </td>
                        <td>
                            <a href="{{ route('admin.track.show', ['uuid' => $track->track_uuid]) }}" class="text-white">
                                {{ $track->hasComposed->title }}
                            </a>
                        </td>
                        <td class="text-center">
                            @if ($track->hasComposed->duration)
                            @time($track->hasComposed->duration)
                            @else
                            00:00:00
                            @endif
                        </td>
                        <td class="text-center">
                            <a href="{{ route('admin.track.show', ['uuid' => $track->uuid]) }}" class="btn btn-sm btn-primary">
                                <i class="fas fa-search" aria-hidden="true" title="@ucfirst(__('app.showData'))"></i>
                            </a>
                        </td>
                    </tr>
                    @endforeach
                </tbody>
            </table>
        </div>
        @endif

        @if (count($contact->hasEpisodes) > 0)
        <h2 class="text-white">
            <i class="fas fa-podcast" aria-hidden="true" title="@ucfirst(__('app.episodesList'))"></i>
            @ucfirst(__('app.episodesList'))
        </h2>

        <div class="table-responsive">
            <table class="table table-dark table-striped table-hover">
                <thead>
                    <tr>
                        <th class="text-center">#</th>
                        <th class="text-center">@ucfirst(__('app.podcastName'))</th>
                        <th class="text-center">@ucfirst(__('app.episodeTitle'))</th>
                        <th class="text-center">@ucfirst(__('app.profession'))</th>
                        <th class="text-center">@ucfirst(__('app.actions'))</th>
                    </tr>
                </thead>
                <tbody>
                    @foreach($contact->hasEpisodes as $episode)
                    <tr>
                        <td class="text-center">{{ $loop->iteration }}</td>
                        <td>
                            <a href="{{ route('admin.podcast.show', ['uuid' => $episode->hasPodcast->uuid]) }}" class="text-white">
                                {{ $episode->hasPodcast->name }}
                            </a>
                        </td>
                        <td>
                            <a href="{{ route('admin.episode.show', ['uuid' => $episode->hasEpisode->uuid]) }}" class="text-white">
                                {{ $episode->hasEpisode->title }}
                            </a>
                        </td>
                        <td>
                            {{ $episode->hasProfession->profession }}
                        </td>
                        <td class="text-center">
                            <a href="{{ route('admin.episode.show', ['uuid' => $episode->uuid]) }}" class="btn btn-sm btn-primary">
                                <i class="fas fa-search" aria-hidden="true" title="@ucfirst(__('app.showData'))"></i>
                            </a>
                        </td>
                    </tr>
                    @endforeach
                </tbody>
            </table>
        </div>
        @endif
    </div>

    <div class="col-4">
        <div class="card mb-4">
            <h5 class="card-header d-flex justify-content-between align-items-center">
                <span>{{ $contact->uname }}</span>
                <span class="badge badge-secondary badge-pill">
                    @if ($contact->gender === 'band')
                    <i class="fas fa-users" aria-hidden="true" aria-label="@ucfirst(__('app.band'))"></i>
                    @elseif ($contact->gender === 'feminine')
                    <i class="fas fa-venus" aria-hidden="true" aria-label="@ucfirst(__('app.feminine'))"></i>
                    @elseif ($contact->gender === 'masculine')
                    <i class="fas fa-mars" aria-hidden="true" aria-label="@ucfirst(__('app.masculine'))"></i>
                    @elseif ($contact->gender === 'neutral')
                    <i class="fas fa-transgender-alt" aria-hidden="true" aria-label="@ucfirst(__('app.neutral'))"></i>
                    @else
                    <i class="fas fa-genderless" aria-hidden="true" aria-label="@ucfirst(__('app.neutral'))"></i>
                    @endif
                </span>
            </h5>
            <ul class="list-group list-group-flush">
                @if ($contact->gender === 'band')
                <li class="list-group-item">
                    @ucfirst(__('app.band')).
                </li>
                @else
                <li class="list-group-item">
                    @ucfirst(__('app.fname')) : {{ $contact->fname }}.<br />
                    @ucfirst(__('app.lname')) : {{ $contact->lname }}.
                </li>
                @endif
                @if ($contact->born_at && $contact->born_on)
                <li class="list-group-item">
                    @if($contact->born_on)
                    @ucfirst(__('app.bornOn')) : @date($contact->born_on).<br />
                    @else
                    @ucfirst(__('app.bornOn')) : @ucfirst(__('app.bornOnUnknown')).<br />
                    @endif
                    @if($contact->born_at)
                    @ucfirst(__('app.bornAt')) : {{ $contact->bornAt->name_eng_common }}.
                    @else
                    @ucfirst(__('app.bornAt')) : @ucfirst(__('app.bornAtUnknown')).
                    @endif
                </li>
                @endif
                @if ($contact->died_at && $contact->died_on)
                <li class="list-group-item">
                    @if ($contact->died_on)
                    @ucfirst(__('app.diedOn')) : @date($contact->died_on).<br />
                    @else
                    @ucfirst(__('app.diedOn')) : @ucfirst(__('app.diedOnUnknown')).<br />
                    @endif
                    @if ($contact->died_at)
                    @ucfirst(__('app.diedAt')) : {{ $contact->diedAt->name_eng_common }}.
                    @else
                    @ucfirst(__('app.diedAt')) : @ucfirst(__('app.diedAtUnknown')).
                    @endif
                </li>
                @endif
                @if ($contact->biography)
                <li class="list-group-item text-justify">
                    {{ $contact->biography }}
                </li>
                @endif
            </ul>
            <div class="card-footer text-right">
                <small class="text-muted">{{ __('app.updatedAt') }} @datetime($contact->updated_at)</small>
            </div>
        </div>
        <div class="card">
            <h5 class="card-header">
                <i class="fas fa-list-ol" aria-hidden="true" title="@ucfirst(__('app.links'))"></i>
                @ucfirst(__('app.sources'))
                @can('create')
                <div class="btn-toolbar mb-2 mb-md-0 float-right">
                    <div class="btn-group">
                        <button type="button" class="btn btn-sm btn-primary" data-toggle="modal" data-target="#modalSourceCreate">
                            <i class="fas fa-link" aria-hidden="true" title="@ucfirst(__('app.sourceStore'))"></i>
                            <span class="sr-only">@ucfirst(__('app.sourceStore'))</span>
                        </button>
                        <a href="{{ route('admin.source.index', ['uuid' => $contact->uuid]) }}" role="button" class="btn btn-sm btn-info">
                            <i class="fas fa-search" aria-hidden="true" title="@ucfirst(__('app.sourceStore'))"></i>
                            <span class="sr-only">@ucfirst(__('app.sourceStore'))</span>
                        </a>
                    </div>
                </div>
                @endcan
            </h5>
            @if (count($contact->hasLinks) > 0)
            <ul class="list-group list-group-flush">
                @foreach($contact->hasLinks as $link)
                <a href="{{ $link->data }}" class="list-group-item list-group-item-action">
                    <i class="{{ $link->info->icon }}" aria-hidden="true" title="@ucfirst(__('app.' . $link->type))"></i>
                    @ucfirst(__('app.' . $link->type))
                </a>
                @endforeach
            </ul>
            <div class="card-footer text-right">
                <small class="text-muted">{{ __('app.updatedAt') }} @datetime($contact->hasLinks->last()->updated_at)</small>
            </div>
            @endif
        </div>
    </div>
</div>
@can('update')
@include('modals.sourceContactCreate')
@endcan
    
@endsection

@section('js')
<script type="text/javascript">
$( function() {
    $("#podcasts").autocomplete({
        source: function (request, response) {
            $.getJSON("{!! route('api.podcast.autocomplete', ['q']) !!}=" + request.term, function (data) {
                if(!data.length){
                    var result = [{
                        label: '@ucfirst(__('app.searchNotFound'))',
                        value: response.term
                    }];
                    response(result);
                }
                else
                {
                    response($.map(data, function (value, key) {
                        return {
                            label: value.name,
                            value: value.name,
                            uuid: value.uuid
                        };
                    }));
                }
            });
        },
		minLength: 3,
		select: function( event, ui ) {
            $("#podcast_uuid").val(ui.item.uuid);
		}
	});
});
</script>
@endsection